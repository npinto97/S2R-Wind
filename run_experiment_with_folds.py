...
if __name__ == '__main__':
    ...
    for fold in range(8):
        arff_path = os.path.join(base_dir, f"fold{fold}.arff")
        file_name = f"swdpf_fold{fold}"

        print(f"\nFold {fold} | Loading {arff_path}")
        ...
        for scale in config['dataset_params']['scale_labeled_samples']:
            ...
            learners = [
                RandomForestRegressor(n_estimators=100, n_jobs=-1, random_state=1),
                RandomForestRegressor(n_estimators=120, n_jobs=-1, random_state=2),
                RandomForestRegressor(n_estimators=150, n_jobs=-1, random_state=3),
            ]
